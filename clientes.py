...
# --- 4. FUNCIONES DE BÚSQUEDA Y TRADUCCIÓN OPTIMIZADA ---

# FIX: Usamos caché para la traducción para evitar el error 'removeChild' en móviles
...
def buscar_producto_supabase(sku_usuario):
    ...
    try:
        # 1. Búsqueda principal (ignorando guiones y mayúsculas)
        ...
        # 2. Búsqueda secundaria (por si en la BD hay guiones)
        if '-' in sku_usuario:
...
if (busqueda_input or boton_consultar):
    if not supabase:
        ...
        if producto:
            # MAPEO DE COLUMNAS (item, descripcion, total_unitario)
            ...
            # Traducción usando la función cacheada (ESTO ARREGLA EL ERROR DEL CELULAR)
